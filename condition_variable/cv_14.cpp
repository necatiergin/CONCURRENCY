...
void waiting_thread() {
    //std::this_thread::sleep_for(std::chrono::milliseconds(300)); // Let notifier go first
    std::this_thread::sleep_for(std::chrono::milliseconds(0)); // Let notifier go first
    std::cout << "[WaitingThread] Trying to acquire lock...\n";

    std::unique_lock<std::mutex> lock(mtx);
    std::cout << "[WaitingThread] Lock acquired. Calling wait...\n";

    cv.wait(lock, predicate); // predicate() will log internally
